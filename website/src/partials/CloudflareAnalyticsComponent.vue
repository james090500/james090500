    ...
    created() {
        this.axios.get('/mccapes').then((response) => {
            if (response.data.error == null) {
                var array =
                    response.data.data.viewer.zones[0].httpRequests1hGroups

                // Make Local Vars
                const dateData = []
                const totalCachedRequests = []
                const totalUncachedRequests = []
                const totalCachedData = []
                const totalUncachedData = []

                // Loop through the data
                for (let i = 0; i < array.length; i++) {
                    dateData.push(
                        new Date(array[i].date.datetime).toLocaleString()
                    )
                    totalCachedRequests.push(array[i].sum.cachedRequests)
                    totalUncachedRequests.push(
                        array[i].sum.requests - array[i].sum.cachedRequests
                    )

                    totalCachedData.push(
                        (
                            array[i].sum.cachedBytes /
                            (1000 * 1000 * 1000)
                        ).toFixed(2)
                    )
                    totalUncachedData.push(
                        (
                            (array[i].sum.bytes - array[i].sum.cachedBytes) /
                            (1000 * 1000 * 1000)
                        ).toFixed(2)
                    )
                }
