<template>
    <section>
        <div class="contactme">
            <h1 class="text-center mb-0">Contact Me</h1>
            <p class="text-center mt-0">Hire, Enquire or Chat</p>
            <div class="pt-10 text-center">
                <button class="btn btn-discord me-3">
                    <font-awesome-icon :icon="['fab', 'discord']" /> james090500
                </button>
                <a
                    href="https://github.com/james090500"
                    target="_blank"
                    class="btn btn-github me-3"
                >
                    <font-awesome-icon :icon="['fab', 'github']" /> james090500
                </a>
                <a
                    href="https://www.linkedin.com/in/james090500/"
                    target="_blank"
                    class="btn btn-linkedin me-3"
                >
                    <font-awesome-icon :icon="['fab', 'linkedin']" /> James
                    Harrison
                </a>
                <a
                    href="mailto:james@fasttortoise.co.uk"
                    class="btn btn-primary"
                >
                    <font-awesome-icon icon="envelope" />
                    james@fasttortoise.co.uk
                </a>
            </div>
        </div>
    </section>
</template>

<style scoped lang="scss">
h1 {
    line-height: 5rem;
    font-size: 7rem;
}

.contactme {
    background-image: url('/img/contactme.jpg');
    background-size: cover;
    background-position: center;
    min-height: 10vh;
    width: 100%;
    padding-top: 5vh;
    padding-bottom: 5vh;
}

.btn {
    margin-bottom: 1rem;
    box-sizing: content-box;
    border-bottom: 0.5rem solid #115ea5 !important;
}

.btn-discord {
    background-color: #5865f2 !important;
    border-color: #454fbf !important;

    &:active {
        background-color: #454fbf !important;
    }
}

.btn-linkedin {
    background-color: #0a66c2 !important;
    border-color: #0959aa !important;

    &:active {
        background-color: #0959aa !important;
    }
}

.btn-github {
    background-color: #333333 !important;
    border-color: #0e0e0e !important;

    &:active {
        background-color: #0e0e0e !important;
    }
}
</style>
